...
db.run('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL, student BOOLEAN NOT NULL)', (err) => {
    if (err) {
      return console.error(err.message);
    }
...
app.post('/signup', (req, res) => {
    ...
    const sql = 'INSERT INTO users (name, email, password, student) VALUES (?, ?, ?, ?)';
    db.run(sql, [name, email, password, student], (err) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
...
app.post('/logout', (req, res) => {
  ...
  app.get('/user', (req, res) => {
    const token = req.query.token;
    const sql = 'SELECT users.id, users.name, users.email, users.student FROM users INNER JOIN session_tokens ON users.id = session_tokens.user_id WHERE session_tokens.token = ?';
    db.get(sql, [token], (err, row) => {
      ...
      return res.json({ id: row.id, name: row.name, email: row.email, student:  row.student});
    });
  });
